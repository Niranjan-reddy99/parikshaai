...
# Model for answer detection — gemini-2.0-flash: cheaper, sufficient for green/red color detection
_VISION_MODEL = "publishers/google/models/gemini-2.5-flash-lite"
...
def process_cbt_job_background(
    ...
    try:
        ...
        for shift_num, shift in enumerate(shifts, 1):
            ...
            # Validation: check if we found the expected number of questions
            if expected_count > 0 and len(merged) != expected_count:
                print(f"  [warn] Shift {shift.shift_label}: found {len(merged)} questions, "
